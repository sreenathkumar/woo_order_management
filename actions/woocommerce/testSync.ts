...
async function testSync() {
    try {
        ...
        //fetch the database orders and compare with the fetched orders
        const dbOrders = await Order.find({ createdAt: { $gte: lastSynced } }).select('-_id -__v -createdAt -updatedAt');

        //check if the fetched orders are already in the database
        const isSame = JSON.stringify(fetchedOrders) === JSON.stringify(dbOrders);

        if (isSame) {
            await SyncTime.findOneAndUpdate({ doc: 'orders' }, { lastSyncedAt: newLastSynced });

            return { status: 'success', message: 'No new orders found' }
        };
        ...
        const existingOrders = await Order.find({ productId: { $in: fetchedOrderIds } }).select('order_id');
        const existingOrderIds = existingOrders.map((order) => order.productId);
        ...
        console.log('notAddedOrders: ', notAddedOrders.length);
